...
# ✅ 게시글 목록 조회(GET) + 게시글 등록(POST) 뷰
# 🔄 ViewSet → ListCreateAPIView 로 전환
class CommunityListCreateView(generics.ListCreateAPIView):
    ...
    # ✅ 요청 메서드에 따라 Serializer 분기
    def get_serializer_class(self):
        if self.request.method == 'POST':
            return CommunityCreateUpdateSerializer
        return CommunitySerializer

    # ✅ 게시판 타입(type) 쿼리 파라미터로 필터링
...
class CommunityDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    # ✅ 수정일 경우 CreateUpdateSerializer 사용
    def get_serializer_class(self):
        if self.request.method in ['PATCH', 'PUT']:
            return CommunityCreateUpdateSerializer
        return CommunitySerializer
...
# ✅ 좋아요 등록/취소 뷰 (POST/DELETE)
# 🔄 ViewSet이 아닌 APIView 사용
