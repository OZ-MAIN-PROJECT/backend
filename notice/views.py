...
class NoticeUpdateView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(NoticeSerializer(self.get_object(), context={"request": request}).data)
...
class NoticeLikeToggleView(APIView):
    ...
    def post(self, request, notice_id):
        ...
        notice.save()
        return Response({"detail": "좋아요 등록", "like_count": notice.like_count, "is_liked": True}, status=status.HTTP_201_CREATED)

    def delete(self, request, notice_id):
        notice = get_object_or_404(Notice, pk=notice_id)
        like = NoticeLike.objects.filter(user=request.user, notice=notice).first()
        ...
        notice.like_count = NoticeLike.objects.filter(notice=notice).count()
        notice.save()
        return Response({"detail": "좋아요 취소", "like_count": notice.like_count, "is_liked": False}, status=status.HTTP_200_OK)
